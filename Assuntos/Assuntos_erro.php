...
$old_error_handler = set_error_handler("Assuntos_scriptcase_error_handler");

function Assuntos_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['Assuntos']['contr_erro']) && $_SESSION['scriptcase']['Assuntos']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_Assuntos->contr_Assuntos->Apl_com_erro = true;
            if ($inicial_Assuntos->contr_Assuntos->NM_ajax_flag)
            {
                $inicial_Assuntos->contr_Assuntos->NM_ajax_info['errList']['geral_Assuntos'][] = $msg;
                return;
            }
            Assuntos_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function Assuntos_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['Assuntos'])  && '' != $_SESSION['scriptcase']['error_icon']['Assuntos'])  ? $_SESSION['scriptcase']['error_icon']['Assuntos']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['Assuntos']) && '' != $_SESSION['scriptcase']['error_close']['Assuntos']) ? $_SESSION['scriptcase']['error_close']['Assuntos'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['Assuntos']) && '' != $_SESSION['scriptcase']['error_span_title']['Assuntos']) ? $_SESSION['scriptcase']['error_span_title']['Assuntos'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['Assuntos']) && '' != $_SESSION['scriptcase']['error_icon_title']['Assuntos']) ? $_SESSION['scriptcase']['error_icon_title']['Assuntos'] : '';
?>
