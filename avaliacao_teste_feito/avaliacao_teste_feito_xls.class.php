...
class avaliacao_teste_feito_xls
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   var $Tit_doc;
   //---- 
   function avaliacao_teste_feito_xls()
   {
   }
   ...
   function inicializa_vars()
   {
      ...
      $this->Arquivo   .= ".xls";
      $this->Tit_doc    = "avaliacao_teste_feito.xls";
   ...
   function grava_arquivo()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['avaliacao_teste_feito']['xls_name']))
      {
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['avaliacao_teste_feito']['xls_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['avaliacao_teste_feito']['xls_name'];
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['avaliacao_teste_feito']['xls_name']);
          $this->Xls_f = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
      }
      ...
      { 
          $SC_Label = (isset($this->New_label['u512969981_questoes_u512969981_questao'])) ? $this->New_label['u512969981_questoes_u512969981_questao'] : "Questões"; 
          if ($Cada_col == "u512969981_questoes_u512969981_questao" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
          ...
          $SC_Label = (isset($this->New_label['resp'])) ? $this->New_label['resp'] : "Resultado"; 
          if ($Cada_col == "resp" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
             if (!NM_is_utf8($SC_Label))
              {
                  $SC_Label = sc_convert_encoding($SC_Label, "UTF-8", $_SESSION['scriptcase']['charset']);
...
$_SESSION['scriptcase']['avaliacao_teste_feito']['contr_erro'] = 'off'; 
   ...
   {
         $this->u512969981_questoes_u512969981_questao = html_entity_decode($this->u512969981_questoes_u512969981_questao, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->u512969981_questoes_u512969981_questao = strip_tags($this->u512969981_questoes_u512969981_questao);
         if (!NM_is_utf8($this->u512969981_questoes_u512969981_questao))
         {
         ...
         $this->Nm_ActiveSheet->getStyle($this->calc_cell($this->Xls_col) . $this->Xls_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
         $this->Nm_ActiveSheet->setCellValueExplicit($this->calc_cell($this->Xls_col) . $this->Xls_row, $this->u512969981_questoes_u512969981_questao, PHPExcel_Cell_DataType::TYPE_STRING);
         $this->Xls_col++;
   }
   //----- resp
   function NM_export_resp()
   {
         $this->resp = html_entity_decode($this->resp, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->resp = strip_tags($this->resp);
         if (!NM_is_utf8($this->resp))
         {
             $this->resp = sc_convert_encoding($this->resp, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->Nm_ActiveSheet->getStyle($this->calc_cell($this->Xls_col) . $this->Xls_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
         $this->Nm_ActiveSheet->setCellValueExplicit($this->calc_cell($this->Xls_col) . $this->Xls_row, $this->resp, PHPExcel_Cell_DataType::TYPE_STRING);
         $this->Xls_col++;
   }
   ...
   function monta_html()
   {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['avaliacao_teste_feito']['xls_file']);
      if (is_file($this->Xls_f))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['avaliacao_teste_feito']['xls_file'] = $this->Xls_f;
      }
...
<HEAD>
 <TITLE>Testes Feitos :: Excel</TITLE>
